...
def apply_ccm(image, ccm, device):
    """Applies a color correction matrix."""
    image = image.permute(1, 2, 0) # Permute the image tensor to HxWxC format from CxHxW format
    shape = image.size()
    image = torch.reshape(image, [-1, 3])
    image = torch.tensordot(image, ccm.to(device), dims=[[-1], [-1]])
...
def unprocess(image):
    ...
    cam2rgb = cam2rgb.to(device)
    rgb_gain = rgb_gain.to(device)
    red_gain = red_gain.to(device)
    blue_gain = blue_gain.to(device)
    ...
    image = apply_ccm(image, rgb2cam, device)
